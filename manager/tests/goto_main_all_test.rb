...
class GotoMainAllTest < Minitest::Test
  def test_pulls_current_repo_and_siblings_with_the_task
    current = add_repo(File.basename(Worktree.root))
    other = add_repo("other.org")
    FileUtils.mkdir_p(File.join(parent, "plain"))
    commands = stub_mise

    output, = capture_io { GotoMainAll.call }

    assert_equal [ current, other ].sort, commands.map { |command| command[:directory] }
    assert_equal "", output
    assert commands.all? { |command| command[:args] == [ "mise", "manager:gotomain" ] }
  end

  def test_skips_repos_without_manager_gotomain
    add_repo("app.org", toml: "[tasks.\"manager:watch\"]\nrun = \"true\"\n")
    commands = stub_mise
    capture_io { GotoMainAll.call }
  ...
  def test_prints_gotomain_output
    add_repo("app.org")
    stub_mise(stdout: "app.org: Already up to date.\n")

    output, = capture_io { GotoMainAll.call }

    assert_equal "app.org: Already up to date.\n", output
  end

  def test_raises_when_gotomain_fails
    add_repo("alpha.org")
    add_repo("zeta.org")
    commands = stub_mise(stderr: "boom", success: false)

    error = assert_raises(RuntimeError) { capture_io { GotoMainAll.call } }

    assert_equal "mise manager:gotomain failed in #{File.join(parent, "alpha.org")}: boom", error.message
  ...
  def test_uses_stdout_when_stderr_is_blank
    ...
    assert_equal "mise manager:gotomain failed in #{File.join(parent, "app.org")}: failed", error.message
  end

  def test_directories_lists_pullable_repos
    current = add_repo(File.basename(Worktree.root))
    other = add_repo("other.org")
    add_repo("skip.org", toml: "")
    add_worktree("other.org-moto-1")

    assert_equal [ current, other ].sort, GotoMainAll.directories
  end
  ...
  def add_repo(name, toml: "[tasks.\"manager:gotomain\"]\nrun = \"true\"\n")
